...
from config import AUTHORIZATION_ID

def getting_token_gigachat() -> str:
...
def monitoring_sber(system_prompt: str, user_prompt: str, answer_models: str, model: str) -> str:
    ...
    if response_monitoring.status_code != 200:
        ...
        if response_balance.status_code != 200:
            ...
            tokens = response_balance.json()["balance"][2]["value"]
            monitoring_stat = f"Использовано {tokens_used} токенов. Остаток токенов на модели {model}: {tokens}"
